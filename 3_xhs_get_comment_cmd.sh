...
Supported input CSVs:
  - accounts.csv generated by ./1_xhs_get_keyword_user_ids.sh and ./2_xhs_get_user_latest_notes.sh
...
INPUT_FILE="${SCRIPT_DIR}/output/latest/accounts.csv"
OUTPUT_FILE="${SCRIPT_DIR}/output/latest/comments.txt"
PREFIX="xhs comment"
SUFFIX="-c hello"
SEARCH_DIR="${SCRIPT_DIR}/output/latest/raw/search"
...
with input_file.open("r", encoding="utf-8-sig", newline="") as src:
    ...
    required_columns = {"user_name", "user_id", "last_note_id", "last_note_title"}
    missing = sorted(required_columns - set(reader.fieldnames or []))
    if missing:
        missing_str = ", ".join(missing)
        raise SystemExit(
            f"Input CSV is missing required columns: {missing_str}. "
            "Expected accounts.csv or a compatible manual_review.csv."
        )
    rows = []
    seen = set()
    for row in reader:
